                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="container p-3 my-3 border">
                                        <div class="h5">Nombre Proceso Compra: {{$contrato->convenio->convenio}}</div>
                                        <div class="h5">Modalidad: {{$contrato->modalidad->nombre_modalidad}}</div>
                                        <div class="h5">ID Proceso Compra: {{$contrato->convenio->id_licitacion}}</div>
                                        ...
                                        <div class="h5">Modificacion Contrato: {{$contrato->aumento_contrato}}</div>
                                        <div class="h5">Res. Modificacion Contrato: {{$contrato->res_aumento}}</div>
                                    </div>
                                    @foreach ($montoboleta as $boletas)
                                        @if ($boletas->id_contrato_original != NULL && $boletas->id_contrato_original == $contrato->id)
                                        <div class="container p-3 my-3 border">
                                            <div class="h5">N° Documento de Garantía: {{$boletas->id_boleta}}</div>
                                            @if ($boletas->tipomoneda->Nombre_tipo == 'CLP')
                                            <div class="h5">Monto Documento Garantía: ${{ number_format($boletas->monto_boleta)}} {{$boletas->tipomoneda->Nombre_tipo}}</div>
                                            @endif
                                            @if ($boletas->tipomoneda->Nombre_tipo == 'USD')
                                            <div class="h5">Monto Documento Garantía: ${{ number_format($boletas->monto_boleta,2,',','.') }} {{$boletas->tipomoneda->Nombre_tipo}}</div>
                                            @endif
                                            @if ($boletas->tipomoneda->Nombre_tipo == 'UF')
                                            <div class="h5">Monto Documento de Garantía: ${{ number_format($boletas->monto_boleta)}} {{$boletas->tipomoneda->Nombre_tipo}}</div>
                                            @endif
                                            <div class="h5">Fecha Vencimiento Documento de Garantía: {{$boletas->fecha_vencimiento}}</div>
                                        </div>
                                        @endif
                                    @endforeach
                                    <div class="row">
